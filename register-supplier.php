    ...
    if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['passwordC'])) {
        ...
        if($password == $passwordC) {
            ...
            if ($result && $result->num_rows == 1) {
                ...
                $stmt = $conn->prepare("INSERT INTO suppliers (supplier_id, password) VALUES (?, ?)");
                $stmt->bind_param("ss", $supplier_id, $password);       
                if ($stmt->execute()) {
                    header("Location: login-supplier.php");
                    exit();
                } else {
...
<html lang="en">
  ...
  <body>
    <div class="page page-login">
      <div class="login">
        <form action="register-supplier.php" method="post">
          <h2>Hello! Welcome👋</h2>
